...
class Menu
{
    ...
    protected $class = '';
    protected $table = 'exam';      //table from DB,which table w`ll select for menu
    protected $cache = 3600;            //valid date for cache = 1 hour(3600 seconds)
    protected $cacheKey = 'idoctor_menu'; //valid date for cache = 1 hour(3600 seconds)
    ...
    protected function run()  //need to add menu in cache || select from DB and add to cache
    {
        ...
        if (!$this->menuHtml) {
            ...
            if (!$this->data) {
                $this->data = $cats = \R::getAssoc("SELECT * FROM {$this->table} {$this->condition}");
            }
    ...
    protected function getMenuHtml($tree, $tab = '')
    {
        ...
        foreach ($tree as $id => $exam) {
            $str .= $this->catToTemplate($exam, $tab, $id);
        }
    ...
    protected function catToTemplate($exam, $tab, $id)
    {  //$id - $id category
        ob_start();
        require $this->tpl;
        return ob_get_clean();
